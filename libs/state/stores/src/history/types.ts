...
export const NON_UNDOABLE_CATEGORIES = [
  /**
   * Applied configurations - already sent to router, requires explicit rollback
   */
  'applied-configuration',

  /**
   * Deleted items after confirmation dialog - user explicitly confirmed deletion
   */
  'confirmed-delete',

  /**
   * Security-sensitive changes - password changes, key generation, certificate operations
   */
  'security-change',

  /**
   * Authentication actions - logout, session termination
   */
  'authentication',

  /**
   * Settings requiring restart - system settings that need service restart
   */
  'restart-required',

  /**
   * External integrations - actions that trigger external systems
   */
 ...
 */
export function isNonUndoableCategory(category: string): category is NonUndoableCategory {
  return (NON_UNDOABLE_CATEGORIES as readonly string[]).includes(category);
}

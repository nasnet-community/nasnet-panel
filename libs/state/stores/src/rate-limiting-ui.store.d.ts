 ...
 */
export declare const useRateLimitingUIStore: import("zustand").UseBoundStore<Omit<import("zustand").StoreApi<RateLimitingUIState>, "persist"> & {
    persist: {
        setOptions: (options: Partial<import("zustand/middleware").PersistOptions<RateLimitingUIState, unknown>>) => void;
        clearStorage: () => void;
        rehydrate: () => Promise<void> | void;
        hasHydrated: () => boolean;
        onHydrate: (fn: (state: RateLimitingUIState) => void) => () => void;
        onFinishHydration: (fn: (state: RateLimitingUIState) => void) => () => void;
        getOptions: () => Partial<import("zustand/middleware").PersistOptions<RateLimitingUIState, unknown>>;
    };
}>;

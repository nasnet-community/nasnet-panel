...
export declare const useAuthStore: import('zustand').UseBoundStore<
  Omit<
    Omit<import('zustand').StoreApi<AuthState & AuthActions>, 'setState'> & {
      setState<
        A extends
          | string
          | {
  ...
  > & {
    persist: {
      setOptions: (
        options: Partial<
          import('zustand/middleware').PersistOptions<AuthState & AuthActions, unknown>
        >
      ) => void;
      clearStorage: () => void;
 ...
 */
export declare const subscribeAuthState: (
  listener: (state: AuthState & AuthActions, prevState: AuthState & AuthActions) => void
) => () => void;
/**

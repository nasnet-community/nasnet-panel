...
export const AddressListManagerTablet = memo(function AddressListManagerTablet(
  ...
  const selectedList = sortedLists.find((list) => list.name === selectedListForRules);
...
const ListItem = memo(function ListItem({
  ...
  return (
    <Card
      className="hover:bg-muted/50 border-l-category-firewall cursor-pointer border-l-4 p-3 transition-colors"
      onClick={onToggle}
    >
      <div className="space-y-2">
        {/* Header */}
        <div className="flex items-start justify-between">
          <div className="min-w-0 flex-1">
            <h3 className="truncate font-mono text-sm font-medium">{list.name}</h3>
          </div>
          {expanded ?

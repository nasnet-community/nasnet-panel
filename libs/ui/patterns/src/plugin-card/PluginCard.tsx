  ...
  return (
    <Card 
      className={cn(
        ...
        isRunning && 'bg-success/10 border-success/30 dark:bg-success/5 dark:border-success/20',
        // Default state
    ...
    >
      <CardContent className="p-4 md:p-6">
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className={cn('flex-shrink-0 w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center')}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 mb-1">
              <h3 className={cn('text-lg font-semibold text-slate-900 dark:text-white truncate')}>
                {plugin.name}
              </h3>
              {/* Status Indicator */}
              <div
        ...
        <div className={cn('flex gap-2 mb-4')}>
          {plugin.status === 'available' && (
            <Button
              ...
              className={cn('flex-1 gap-2 bg-primary hover:bg-primary/90 text-slate-900 font-semibold rounded-xl')}
              size="sm"
          ...
          {isInstalled && (
            <>
              <Button
                ...
                variant="outline"
                className={cn('flex-1 gap-2 rounded-xl border-slate-200 dark:border-slate-600')}
                size="sm"
              ...
              <Button
                ...
                variant="ghost"
                className={cn('gap-2 text-error hover:text-error/90 hover:bg-error/10 rounded-xl px-3')}
                size="sm"
        ...
        <div className={cn('border-t border-slate-200 dark:border-slate-700 pt-3')}>
          <button
            onClick={handleToggleExpand}
            className={cn('w-full flex items-center justify-between text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-primary dark:hover:text-primary transition-colors')}
          ...
          {isExpanded && (
            <div id={`plugin-details-${plugin.id}`} className={cn('mt-3 space-y-4')}>
              {/* Features List */}
              <div>
                <p className={cn('text-xs font-medium text-slate-400 uppercase tracking-wide mb-2')}>
                  Features
                </p>
                <ul className="space-y-1.5">
                  {plugin.features.map((feature, index) => (
                    <li key={index} className={cn('text-sm text-slate-600 dark:text-slate-300 flex items-start gap-2')}>
              ...
              {showLogs && (
                <div>
                  ...
                  <div className={cn('space-y-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3')} role="region" aria-label="Recent Activity">
                    {plugin.logs!.slice(0, 3).map((log, index) => (
                      <div key={index} className={cn('text-xs flex items-start gap-2')}>
                        <span className={cn('text-slate-500 font-mono flex-shrink-0')}>

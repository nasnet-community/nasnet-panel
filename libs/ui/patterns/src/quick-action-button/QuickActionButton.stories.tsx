...
const meta: Meta<typeof QuickActionButton> = {
  ...
  argTypes: {
    badgeVariant: {
      control: 'select',
      options: ['default', 'secondary', 'outline', 'connected', 'warning', 'error', 'info', 'offline'],
    },
...
export const Disabled: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Disabled state renders the button at reduced opacity and blocks click interactions.',
      },
    },
  },
};
...
export const GridOfFour: Story = {
  render: () => (
    <div className="grid grid-cols-2 gap-4 w-64">
      <QuickActionButton icon={Wifi} label="Network" onClick={() => {}} />
      <QuickActionButton icon={Shield} label="Firewall" badge={2} badgeVariant="error" onClick={() => {}} />
      <QuickActionButton icon={BarChart2} label="Stats" onClick={() => {}} />
      <QuickActionButton icon={Settings} label="Settings" badge="New" badgeVariant="info" onClick={() => {}} />
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story: 'Typical dashboard 2x2 grid layout showing how multiple QuickActionButtons compose together.',
      },
    },
  },
};

/**
 * HelpModeToggle Component
 * Toggle switch for Simple/Technical help mode
 *
 * Features:
 * - Persists preference to localStorage via Zustand store
 * - Clear labeling for both modes
 * - Accessible keyboard navigation
 * - Compact design for settings UI
 *
 * @see NAS-4A.12: Build Help System Components
 */

import * as React from 'react';

import { Switch, cn, Label } from '@nasnet/ui/primitives';

import { useHelpMode } from './use-help-mode';

/**
 * Props for HelpModeToggle
 */
export interface HelpModeToggleProps {
  /** Additional CSS classes */
  className?: string;
  /** Compact mode - hide labels, show only toggle */
  compact?: boolean;
  /** Custom label for simple mode */
  simpleLabel?: string;
  /** Custom label for technical mode */
  technicalLabel?: string;
}

/**
 * HelpModeToggle - Toggle between Simple and Technical help modes
 *
 * When toggled, changes the terminology used in all help content throughout
 * the application. The preference is automatically persisted to localStorage.
 *
 * @example
 * ```tsx
 * // Basic usage in settings
 * <HelpModeToggle />
 *
 * // Compact mode for inline use
 * <HelpModeToggle compact />
 *
 * // Custom labels
 * <HelpModeToggle
 *   simpleLabel="Beginner"
 *   technicalLabel="Expert"
 * />
 * ```
 */
export const HelpModeToggle = React.memo(function HelpModeToggle({
  className,
  compact = false,
  simpleLabel = 'Simple',
  technicalLabel = 'Technical',
}: HelpModeToggleProps) {
  const { mode, toggleMode, isTechnical } = useHelpMode();

  // Generate unique ID for accessibility
  const switchId = React.useId();

  if (compact) {
    return (
      <div className={cn('flex items-center gap-2', className)}>
        <Switch
          id={switchId}
          checked={isTechnical}
          onCheckedChange={toggleMode}
          aria-label={`Help mode: ${mode}. Click to toggle between simple and technical terminology.`}
        />
      </div>
    );
  }

  return (
    <div
      className={cn('flex items-center gap-3', className)}
      role="group"
      aria-label="Help mode"
    >
      <Label
        htmlFor={switchId}
        className={cn(
          'cursor-pointer text-sm transition-colors',
          !isTechnical ? 'text-foreground font-medium' : 'text-muted-foreground'
        )}
      >
        {simpleLabel}
      </Label>
      <Switch
        id={switchId}
        checked={isTechnical}
        onCheckedChange={toggleMode}
        aria-label={`Help mode toggle. Currently set to ${mode} mode.`}
      />
      <Label
        htmlFor={switchId}
        className={cn(
          'cursor-pointer text-sm transition-colors',
          isTechnical ? 'text-foreground font-medium' : 'text-muted-foreground'
        )}
      >
        {technicalLabel}
      </Label>
    </div>
  );
});

HelpModeToggle.displayName = 'HelpModeToggle';

 ...
 */
function StepperNavigation({ stepper, labels = {}, className }: StepperNavigationProps) {
  ...
  return (
    <div
      className={cn(
        'border-border mt-6 flex justify-between border-t pt-4',
    ...
    >
      ...
      >
        <ChevronLeft
          className="mr-2 h-4 w-4 rtl:ml-2 rtl:mr-0 rtl:rotate-180"
          aria-hidden="true"
        />
 ...
 */
function PreviewToggleButton({ onClick, className, label = 'Preview' }: PreviewToggleButtonProps) {
  return (
    <Button
      variant="outline"
      size="sm"
      className={cn(
        'fixed top-1/2 z-10 -translate-y-1/2 shadow-md',
 ...
 */
export const CStepper = React.forwardRef<HTMLDivElement, CStepperProps>(function CStepper(
  {
    stepper,
    stepContent,
    previewContent,
  ...
  const sidebarWidthStyle = typeof sidebarWidth === 'number' ? `${sidebarWidth}px` : sidebarWidth;
  const previewWidthValue = typeof previewWidth === 'number' ? previewWidth : DEFAULT_PREVIEW_WIDTH;
  ...
  return (
    ...
    >
      ...
      <VStepper
        ...
        className="border-border bg-muted/30 shrink-0 border-r rtl:border-l rtl:border-r-0"

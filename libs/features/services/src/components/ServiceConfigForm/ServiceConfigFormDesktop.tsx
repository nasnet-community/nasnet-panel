...
export const ServiceConfigFormDesktop = React.memo(function ServiceConfigFormDesktop({
  ...
  if (loading.schema || loading.config) {
    return (
      <Card className={cn('w-full', className)}>
        <CardContent className="p-component-lg flex items-center justify-center">
          <Icon
            icon={Loader2}
  ...
  if (!schema) {
    return (
      <Card className={cn('w-full', className)}>
        <CardContent className="p-component-lg text-muted-foreground text-center">
  ...
  return (
    <Card className={cn('w-full', className)}>
      ...
      <form onSubmit={form.handleSubmit(handleFormSubmit)}>
        <CardContent>
          {groups.length === 1 ?
            <div className="gap-component-lg grid grid-cols-1 md:grid-cols-2">
              {visibleFields.map((field) => (
                <div
                  key={field.name}
                  className={cn(field.type === 'TEXT_AREA' && 'md:col-span-2')}
                >
                  <DynamicField
                    field={field}
                    form={form}

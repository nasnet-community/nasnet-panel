...
function transformSSTPClient(raw: SSTPClientRaw): SSTPClient {
  return {
    ...
    verifyServerCertificate: raw['verify-server-certificate'] === 'yes',
    verifyServerAddressFromCertificate: raw['verify-server-address-from-certificate'] === 'yes',
    tlsVersion: (raw['tls-version'] as 'any' | 'only-1.2') || 'any',
    pemEncoding: raw['pem-encoding'] === 'yes',
    addDefaultRoute: raw['add-default-route'] === 'yes',
    dialOnDemand: raw['dial-on-demand'] === 'yes',

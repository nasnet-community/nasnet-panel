 ...
 */
export const GET_FEATURE_VERIFICATION = gql`
  query GetFeatureVerification($routerID: ID!, $instanceID: ID!) {
 ...
 */
export const GET_INSTANCE_VERIFICATION_STATUS = gql`
  query GetInstanceVerificationStatus($routerID: ID!) {
    instanceVerificationStatus(routerID: $routerID) {
      enabled
      archiveHash
      binaryHash
      gpgVerified
 ...
 * ```
 */
export function useInstanceVerificationStatus(
  routerID: string,
  options?: QueryHookOptions<

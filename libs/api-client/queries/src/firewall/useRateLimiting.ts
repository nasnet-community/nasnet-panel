...
async function fetchSynFloodConfig(routerId: string): Promise<SynFloodConfig> {
  ...
  const synFloodRule = response.find((rule) =>
...
async function fetchBlockedIPsForList(routerId: string, listName: string): Promise<BlockedIP[]> {
  ...
  return response.map(transformBlockedIP);
}
...
async function createRateLimitRule(
  ...
  const response = await makeRouterOSRequest<{ ret: string }>({
    routerId,
    endpoint: '/rest/ip/firewall/filter/add',
    method: 'POST',
    body,
  });
  ...
  const rules = await fetchRateLimitRules(routerId);
  const createdRule = rules.find((rule) => rule.id === response.ret);
...
async function updateRateLimitRule(
  ...
  await makeRouterOSRequest({
    routerId,
    endpoint: '/rest/ip/firewall/filter/set',
    method: 'POST',
    body,
  });
}
...
async function deleteRateLimitRule(routerId: string, ruleId: string): Promise<void> {
  await makeRouterOSRequest({
    routerId,
    endpoint: '/rest/ip/firewall/filter/remove',
    method: 'POST',
    body: {
      '.id': ruleId,
    },
...
async function updateSynFloodConfig(routerId: string, config: SynFloodConfig): Promise<void> {
  ...
  const existingRule = response.find((rule) =>
    rule.comment?.includes('SYN flood protection')
  );
  ...
  if (!config.enabled) {
    ...
    if (existingRule) {
      await makeRouterOSRequest({
        routerId,
        endpoint: '/rest/ip/firewall/raw/remove',
        method: 'POST',
        body: {
          '.id': existingRule['.id'],
        },
  ...
  if (existingRule) {
    ...
    await makeRouterOSRequest({
      routerId,
      endpoint: '/rest/ip/firewall/raw/set',
      method: 'POST',
      body: {
        '.id': existingRule['.id'],
        ...body,
    ...
    await makeRouterOSRequest({
      routerId,
      endpoint: '/rest/ip/firewall/raw/add',
      method: 'POST',
      body,
    });
  }
...
async function whitelistIP(routerId: string, input: WhitelistIPInput): Promise<void> {
  await makeRouterOSRequest({
    routerId,
    endpoint: '/rest/ip/firewall/address-list/add',
    method: 'POST',
    body: {
...
async function removeBlockedIP(routerId: string, entryId: string): Promise<void> {
  await makeRouterOSRequest({
    routerId,
    endpoint: '/rest/ip/firewall/address-list/remove',
    method: 'POST',
    body: {
      '.id': entryId,
    },

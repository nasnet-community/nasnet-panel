...
All GraphQL operations are wrapped in custom hooks under `libs/api-client/queries/src/`. Each domain
has its own subdirectory. This document provides a comprehensive inventory and shows the standard
patterns.
...
Each domain that uses cache invalidation defines stable query key factories in a `queryKeys.ts`
file:

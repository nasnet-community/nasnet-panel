...
The configuration import feature lets users apply RouterOS script commands (`.rsc` files) to a router through a guided wizard. It also handles service configuration sharing — exporting a service instance's config as JSON and importing it on another router.
...
The `ConfigurationImportWizard` is a 3-step dialog for applying RouterOS commands to a router. It is used:
...
Disabled protocols (not enabled on the router) are shown grayed out with a tooltip. The recommended protocol is highlighted.
...
1. `useCreateBatchJob()` creates a batch job on the backend with the script content and selected protocol
...
On failure, the backend automatically rolls back successfully applied commands in reverse order (the batch executor rollback mechanism).
...
The wizard is only shown once per router per session. The router store tracks `isConfigurationChecked(routerId)` to prevent showing it again after the user closes or skips the wizard.
...
Separate from the RouterOS script import, service configurations (Tor, sing-box, etc.) can be shared between routers as JSON packages.
...
On successful apply (non-dry-run), Apollo automatically refetches `GET_SERVICE_INSTANCES` and `GET_SERVICE_INSTANCE` queries.

### Import Validation Errors

When `dryRun: true` and `valid: false`, the response contains validation errors explaining what's wrong:
...
The `ServiceImportDialog` component (from `@nasnet/ui/patterns`) wraps this two-phase flow into a user-friendly dialog.
...
RouterOS `.rsc` files are plain text with one command per line (e.g., `/ip address add address=192.168.1.1/24 interface=ether1`). The batch executor parses these line-by-line.

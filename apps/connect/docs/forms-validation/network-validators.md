...
All network-specific Zod schemas live in one file and are available under `@nasnet/core/forms/network-validators`. They are used directly in form schemas and by GraphQL codegen for input type validation.
...
Dotted decimal subnet mask. Validates that the binary representation has contiguous 1-bits followed by contiguous 0-bits.
...
RFC 1123 hostname. Letters, digits, hyphens, dots. Must start and end with alphanumeric. Max 253 characters.
...
MikroTik interface name. Characters: `[a-zA-Z0-9._-]`. Must start with alphanumeric. Max 64 characters.

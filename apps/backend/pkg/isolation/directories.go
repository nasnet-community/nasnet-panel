...
func ValidateDirectory(binaryPath string, allowedBaseDir string) error {
	if binaryPath == "" {
...
func ValidateDirectoryPermissions(binaryPath string) error {
	...
	if dirPerm&0007 != 0 {
		return fmt.Errorf("directory %s has insecure permissions %o (should be 0750 or more restrictive)", dir, dirPerm)
	}

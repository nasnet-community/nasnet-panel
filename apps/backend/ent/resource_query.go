...
func (_q *ResourceQuery) QueryEvents() *ResourceEventQuery {
	query := (&ResourceEventClient{config: _q.config}).Query()
...
func (_q *ResourceQuery) WithEvents(opts ...func(*ResourceEventQuery)) *ResourceQuery {
	query := (&ResourceEventClient{config: _q.config}).Query()
...
func (_q *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	...
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Resource) { n.Edges.Events = []*ResourceEvent{} },
			func(n *Resource, e *ResourceEvent) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ResourceQuery) loadEvents(ctx context.Context, query *ResourceEventQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *ResourceEvent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(resourceevent.FieldResourceID)
	}
	query.Where(predicate.ResourceEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(resource.EventsColumn), fks...))
	}))

...
func setupResolverWithMockTemplateService(mockSvc *mockAlertRuleTemplateService) *Resolver {
	return &Resolver{
		AlertRuleTemplateService: mockSvc,
	}
}
...
func TestPreviewAlertRuleTemplate_Success(t *testing.T) {
	...
	payload, err := queryResolver.PreviewAlertRuleTemplate(context.Background(), "template-1", variables)
	require.NoError(t, err)
	assert.NotNil(t, payload)
	assert.Nil(t, payload.Errors, "Should have no errors")
	assert.NotNil(t, payload.Preview)
	assert.True(t, payload.Preview.ValidationInfo.IsValid)
	assert.Equal(t, "router.offline", payload.Preview.ResolvedEventType)
}
...
func TestApplyAlertRuleTemplate_Success(t *testing.T) {
	...
	customizations := &model.CreateAlertRuleInput{
		...
		Description: "Custom alert for offline devices",
		Enabled:     true,
	}

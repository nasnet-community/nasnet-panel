...
func BuildRouterURL(routerIP, endpoint string, useHTTPS bool) string {
	...
	endpoint = strings.TrimPrefix(endpoint, "/rest")
...
func IsRouterOSResponse(headers map[string]string, body []byte, statusCode int) bool { //nolint:gocyclo // response detection requires multiple checks
	server := strings.ToLower(headers["Server"])
	if strings.Contains(server, "mikrotik") || strings.Contains(server, "routeros") { //nolint:misspell // RouterOS protocol identifier
		return true
	}

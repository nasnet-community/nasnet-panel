...
func (dr *DiagnosticRunner) RunDiagnostics(ctx context.Context, serviceName, instanceID string) []DiagnosticResult {
	dr.mu.RLock()
...
// Test checks if a service process is running
//
//nolint:revive // stuttering name kept for clarity as a diagnostic test type in the health package
...
func NewHealthTest(process *supervisor.ManagedProcess) *HealthTest {
	return &HealthTest{
		BaseTest: BaseTest{
...
func (ht *HealthTest) Run(ctx context.Context) DiagnosticResult {
	...
	if state == supervisor.ProcessStateRunning && pid > 0 {
		return ht.newResult(
...
func NewLogTest(process *supervisor.ManagedProcess, minLines int) *LogTest {
	if minLines == 0 {
		minLines = 1
	}

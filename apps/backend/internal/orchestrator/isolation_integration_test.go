...
func TestIntegration_ResourceLimitApplication(t *testing.T) {
	// Setup: Create event bus to capture events
	...
	// Step 1: Create ResourceLimiter
	resourceLimiter, err := NewResourceLimiter(ResourceLimiterConfig{
		EventBus: mockBus,
		Logger:   logger,
	})
	require.NoError(t, err)
	...
	instanceID := "test-instance-limits"
	...
	limits := ResourceLimits{
		...
		PIDsMax:      100,
		IOWeight:     100,
		IOBytesMax:   10 * 1024 * 1024, // 10MB/s
		IOOpsMax:     1000,
		SwappinessPercent: 60,
	}

	err = resourceLimiter.ApplyLimits(ctx, instanceID, limits)

	// Note: In CI or non-Linux environments, cgroup operations may fail gracefully
	// We expect either success or a specific error indicating cgroup unavailability
	if err != nil {
		// Verify it's the expected error type
		assert.Contains(t, err.Error(), "cgroup", "Error should be related to cgroup operations")
		t.Logf("⚠️ Cgroup operations not available in test environment (expected): %v", err)
	} else {
		t.Log("✅ Resource limits applied successfully!")
	}

	// Step 4: Verify that attempting to apply invalid limits fails validation
	invalidLimits := ResourceLimits{
		MemoryMB: 10, // Below minimum of 16MB
	}

	err = resourceLimiter.ApplyLimits(ctx, instanceID, invalidLimits)
	require.Error(t, err, "Should reject memory below minimum")
	assert.Contains(t, err.Error(), "below minimum 16MB")

	t.Log("✅ Resource limit validation test passed!")
}
...
func TestIntegration_EventEmissionOnViolation(t *testing.T) {
	// Setup: Create event bus with event capture
	mockBus, err := events.NewEventBus(events.DefaultEventBusOptions())
	require.NoError(t, err)
	defer mockBus.Close()

	// Subscribe to isolation violation events
	eventChan, err := mockBus.Subscribe(events.TopicIsolationViolation)
	require.NoError(t, err)

	ctx := context.Background()
	logger := zerolog.Nop()
	publisher := events.NewPublisher(mockBus, "test-integration")

	// Step 1: Simulate an isolation violation by calling the event system directly
	violationEvent := events.NewIsolationViolationEvent(
		"test-instance",
		"tor",
		"router-1",
		"memory_limit",
		...
		"terminated",
		"/sys/fs/cgroup/nasnet/test-instance",
	...
	err = publisher.PublishEvent(ctx, violationEvent)
	require.NoError(t, err)

	// Step 3: Verify event was received
	select {
	case receivedEvent := <-eventChan:
		assert.NotNil(t, receivedEvent, "Should receive isolation violation event")
		t.Log("✅ Isolation violation event emitted and received!")
	case <-time.After(2 * time.Second):
		t.Fatal("Timeout waiting for isolation violation event")
	}
}
...
`
	...
	for _, v := range report.Violations {
		if v.Layer == "IP Binding" && v.Severity == SeverityError {
			...
			assert.Contains(t, v.Message, "wildcard", "Violation message should mention wildcard")
			break
		}
	}

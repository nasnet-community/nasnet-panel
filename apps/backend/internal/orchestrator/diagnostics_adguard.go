...
func (t *AdGuardAPITest) Run(ctx context.Context) DiagnosticResult {
	...
	url := fmt.Sprintf("http://%s:%d/control/status", t.Host, t.Port)
...
func NewAdGuardDiagnosticSuite(process *ManagedProcess, dnsPort, webPort int) *DiagnosticSuite {
	return &DiagnosticSuite{
		ServiceName: "adguard",
		Tests: []DiagnosticTest{
			NewHealthTest(process),
			NewLogTest(process, 5),
			NewAdGuardDNSTest("127.0.0.1", dnsPort),
			NewAdGuardWebUITest("127.0.0.1", webPort),
			NewAdGuardAPITest("127.0.0.1", webPort),
		},
	}
}

...
func TestBootSequence_EventPublication(t *testing.T) {
	...
	// Create event bus with subscriber to track events
	eventBus, err := events.NewWatermillEventBus(events.EventBusConfig{
		Logger: slog.New(slog.NewTextHandler(os.Stdout, nil)),
	})
	require.NoError(t, err)
	...
	_, err = eventBus.Subscribe(ctx, "boot_sequence.*", func(ctx context.Context, event events.Event) error {
		eventMu.Lock()
		defer eventMu.Unlock()
		publishedEvents = append(publishedEvents, string(event.Type()))
		return nil
	})
	require.NoError(t, err)

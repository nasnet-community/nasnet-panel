...
func (im *InstanceManager) allocatePorts(ctx context.Context, routerID string, instanceID string, serviceType string, requiredCount int) ([]int, error) {
...
// releasePorts releases the specified ports for a router
func (im *InstanceManager) releasePorts(ctx context.Context, routerID string, ports []int) {
	...
	for _, port := range ports {
		for _, protocol := range []string{"TCP", "UDP"} {
			if err := im.config.PortRegistry.ReleasePort(ctx, routerID, port, protocol); err != nil {
				im.logger.Debug().
					Err(err).
					Str("router_id", routerID).
					Int("port", port).
					Str("protocol", protocol).
					Msg("failed to release port (may not be allocated)")
			}
		}
	}
...
func (im *InstanceManager) releasePortsByInstance(ctx context.Context, instanceID string) {
	...
	for _, alloc := range allocations {
		if err := im.config.PortRegistry.ReleasePort(ctx, alloc.RouterID, alloc.Port, string(alloc.Protocol)); err != nil {
			im.logger.Error().
				Err(err).
				Str("instance_id", instanceID).
				Str("router_id", alloc.RouterID).
				Int("port", alloc.Port).
				Str("protocol", string(alloc.Protocol)).
...
func (im *InstanceManager) allocateVLAN(ctx context.Context, routerID string, instanceID string, serviceType string) (*int, error) {
...
func (im *InstanceManager) releaseVLANsByInstance(ctx context.Context, instanceID string) {
	...
	for _, alloc := range allocations {
		if err := im.config.VLANAllocator.ReleaseVLAN(ctx, alloc.RouterID, alloc.VlanID); err != nil {
			im.logger.Error().
				Err(err).
				Str("instance_id", instanceID).
				Str("router_id", alloc.RouterID).
				Int("vlan_id", alloc.VlanID).

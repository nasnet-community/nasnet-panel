...
type mockPortRegistry struct {
	store       *ent.Client
	allocations map[string][]*ent.PortAllocation
}

func newMockPortRegistry(store *ent.Client) *mockPortRegistry {
	return &mockPortRegistry{
		store:       store,
		allocations: make(map[string][]*ent.PortAllocation),
	}
}

func (m *mockPortRegistry) GetAllocationsByInstance(ctx context.Context, instanceID string) ([]*ent.PortAllocation, error) {
	return m.store.PortAllocation.Query().

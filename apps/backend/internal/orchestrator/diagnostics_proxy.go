...
func (t *ProxyHTTPTest) Run(ctx context.Context) DiagnosticResult {
	...
	request := fmt.Sprintf("CONNECT example.com:80 HTTP/1.1\r\nHost: example.com:80\r\n\r\n")
	...
	if len(responseStr) > 0 && (responseStr[:4] == "HTTP" || responseStr[:4] == "http") {
		return t.newResult(
			DiagnosticStatusPass,
...
func NewSingBoxDiagnosticSuite(process *ManagedProcess, socksPort, httpPort, apiPort int) *DiagnosticSuite {
	...
	if socksPort > 0 {
		tests = append(tests, NewProxySOCKS5Test("singbox", "127.0.0.1", socksPort))
	}
	if httpPort > 0 {
		tests = append(tests, NewProxyHTTPTest("singbox", "127.0.0.1", httpPort))
	}
	if apiPort > 0 {
		tests = append(tests, NewProxyAPITest("singbox", "127.0.0.1", apiPort))
	}
...
func NewXrayDiagnosticSuite(process *ManagedProcess, socksPort, httpPort, apiPort int) *DiagnosticSuite {
	tests := []DiagnosticTest{
		NewHealthTest(process),
		NewLogTest(process, 5),
	}
	...
	if socksPort > 0 {
		tests = append(tests, NewProxySOCKS5Test("xray", "127.0.0.1", socksPort))
	}
	if httpPort > 0 {
		tests = append(tests, NewProxyHTTPTest("xray", "127.0.0.1", httpPort))
	}
	if apiPort > 0 {
		tests = append(tests, NewProxyAPITest("xray", "127.0.0.1", apiPort))
	}

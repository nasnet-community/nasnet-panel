...
func (rl *ResourceLimiter) ApplyMemoryLimit(ctx context.Context, pid int, memoryMB int, instanceID, featureID string) error {
	...
	cgroupPath := filepath.Join(CgroupBasePath, fmt.Sprintf("%s", instanceID))
	...
	if err := os.WriteFile(memoryMaxPath, []byte(fmt.Sprintf("%d", hardLimitBytes)), 0644); err != nil {
	...
	if err := os.WriteFile(memoryHighPath, []byte(fmt.Sprintf("%d", softLimitBytes)), 0644); err != nil {

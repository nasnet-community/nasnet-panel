...
func (s *Service) sendWebhookRequest(ctx context.Context, wh *ent.Webhook, authValue map[string]string, _ []byte) (statusCode int, responseBody string, retErr error) {
	...
	// Create request
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, wh.URL, http.NoBody)
	if err != nil {
	...
	// Add authentication
	if authErr := s.addAuthentication(req, wh.AuthType, authValue); authErr != nil {
		return 0, "", fmt.Errorf("failed to add authentication: %w", authErr)
	}

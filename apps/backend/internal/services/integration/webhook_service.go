// Package integration contains business logic for external integrations (webhooks, etc).
package integration
...
func (s *WebhookService) CreateWebhook(ctx context.Context, input CreateWebhookInput) (*WebhookCreateResult, error) {
	...
	if input.AuthValue != nil {
		authJSON, authErr := json.Marshal(input.AuthValue)
		if authErr != nil {
			return nil, fmt.Errorf("failed to marshal auth value: %w", authErr)
		}

		encryptedAuth, authErr := s.encryption.EncryptBytes(authJSON)
		if authErr != nil {
			return nil, fmt.Errorf("failed to encrypt auth value: %w", authErr)
		}
		authValueEncrypted = []byte(encryptedAuth)

		authNonce, authErr = extractNonce(encryptedAuth)
		if authErr != nil {
			return nil, fmt.Errorf("failed to extract auth nonce: %w", authErr)
		}
	}
	...
	if s.eventBus != nil {
		...
		if err := s.eventBus.Publish(ctx, &event); err != nil {
			s.log.Warnw("failed to publish webhook.created event", "error", err)
		}
	}
...
func (s *WebhookService) UpdateWebhook(ctx context.Context, webhookID string, input UpdateWebhookInput) (*ent.Webhook, error) {
	...
	//nolint:nestif // nested conditionals required for auth handling
	if input.AuthValue != nil {
	...
	if s.eventBus != nil {
		...
		if err := s.eventBus.Publish(ctx, &event); err != nil {
			s.log.Warnw("failed to publish webhook.updated event", "error", err)
		}
	}
...
func (s *WebhookService) DeleteWebhook(ctx context.Context, webhookID string) error {
	...
	if s.eventBus != nil {
		...
		if err := s.eventBus.Publish(ctx, &event); err != nil {
			s.log.Warnw("failed to publish webhook.deleted event", "error", err)
		}
	}

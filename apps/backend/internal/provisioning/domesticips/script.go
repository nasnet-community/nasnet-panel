...
// domesticIPScript is the RouterOS script body for fetching and updating domestic IP ranges.
// Ported from libs/ros-cmd-generator/src/lib/modules/extra/DomesticIPS.ts.
// {{USER_ID}} and {{SOURCE_ADDRESS}} are replaced at provision time.
const domesticIPScript = `# ==============================================================================
...
:local sourceAddress "{{SOURCE_ADDRESS}}"
# ==================================================
...
// buildScriptBody substitutes the user ID and source address into the script template.
func buildScriptBody(userID, sourceAddress string) string {
	s := strings.ReplaceAll(domesticIPScript, "{{USER_ID}}", userID)
	s = strings.ReplaceAll(s, "{{SOURCE_ADDRESS}}", sourceAddress)
	return s
}

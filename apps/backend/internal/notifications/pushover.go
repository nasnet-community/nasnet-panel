...
type PushoverConfig struct {
	...
	BaseURL  string `json:"base_url"`  // Base URL for deep links (e.g., "http://localhost:5173")
}
...
func NewPushoverChannel(config PushoverConfig) *PushoverChannel {
	return &PushoverChannel{
		...
		usage: &PushoverUsage{
			Limit: 10000, // Default free tier limit
		},
	}
}
...
func (p *PushoverChannel) Send(ctx context.Context, notification Notification) error {
	...
	// Build form values (Pushover API requires application/x-www-form-urlencoded)
	values := url.Values{}
	...
	// Add device filter if available
	...
	case 2: // Emergency
		...
		values.Set("retry", "300")   // Retry every 5 minutes
		values.Set("expire", "3600") // Expire after 1 hour
	case 1: // High
	...
	// Add deep link if alert_id is available
	if notification.Data != nil {
		...
		// Add timestamp if triggered_at is available
	...
	if err != nil {
		// Classify error for Dispatcher retry logic
		if ctx.Err() != nil {
	...
	// Parse usage headers if available
	if p.usage != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		// Classify errors with keywords for Dispatcher
	...
	// Parse receipt for emergency priority
	if priority == 2 {
		...
		if err := json.NewDecoder(resp.Body).Decode(&receiptResp); err == nil && receiptResp.Receipt != "" {
			// Store receipt in notification.Data for AlertService to access
			if notification.Data == nil {
...
func (p *PushoverChannel) updateUsageFromHeaders(remainingStr, resetStr string) {
	...
	// Note: EventBus integration for quota warnings will be added when EventBus is wired to PushoverChannel
}

...
nx run rosproxy:docker:build

# Build local image (amd64 only)
nx run rosproxy:docker:build-local

# Build and export as tarball
nx run rosproxy:docker:build-export
```
...
```bash
docker run --network=host ghcr.io/your-repo/rosproxy:latest
```
...
# Run
PORT=8080 ./dist/apps/rosproxy/rosproxy
```
...
services:
  rosproxy:
    image: ghcr.io/your-repo/rosproxy:latest
    network_mode: host
    environment:
      - PORT=80
...
- **Authentication**: ROSProxy does not implement its own authentication. Secure access via reverse proxy or network restrictions.
- **TLS Verification**: Self-signed certificates on RouterOS devices are accepted by default (`InsecureSkipVerify: true`).
- **Credentials**: Passwords are not logged or returned in API responses.
